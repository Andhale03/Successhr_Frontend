...
import { BriefcaseBusiness, CheckCircle2, Loader2, MapPin, ShieldCheck, UserRound } from 'lucide-react'
      ...
      <form onSubmit={submit} className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-5">
          <FormSection icon={<ShieldCheck className="h-5 w-5" />} title="Submission Route">
            ...
            {advisor ? (
              <p className="mt-3 text-sm font-semibold text-emerald-700">Valid code for: {advisor.advisorName}</p>
            ) : null}
            {codeError ? <p className="mt-3 text-sm font-semibold text-rose-600">{codeError}</p> : null}
            {!advisorCode ? (
              <p className="mt-3 text-sm text-slate-600">Without advisor code, your form will go directly to candidate management.</p>
            ) : null}
          </FormSection>
        ...
        <header className="mb-5 rounded-xl bg-white px-4 py-4 shadow-sm ring-1 ring-slate-200 sm:px-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {advisor ? (
                <span className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-800 ring-1 ring-cyan-200">{advisor.advisorCode}</span>
              ) : null}
